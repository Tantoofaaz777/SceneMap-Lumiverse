...
function renderField(field: TrackerBoardField, tracker: unknown): string {
  ...
  if (display === "progress") return renderProgressField(label, value);
...
function renderProgressField(label: string, value: unknown): string {
  const progress = parseProgressValue(value);
  if (!progress) return `<div class="scenemap-field"><span>${label}</span><p>${escapeHtml(formatDisplayValue(value))}</p></div>`;
