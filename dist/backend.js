...
var schemaValidator = new import_ajv.default({
  allErrors: true,
  strict: false,
  validateFormats: true
});
import_ajv_formats.default(schemaValidator);
...
function formatSchemaErrors(errors) {
  const messages = errors.slice(0, 6).map((error) => {
    ...
    const missingPath = typeof missingProperty === "string" ? `${error.instancePath}/${escapeJsonPointerToken(missingProperty)}` : error.instancePath;
    const path = missingPath || "/";
    return `${path} ${error.message ?? "is invalid"}`;
  });
  if (errors.length > messages.length)
    messages.push(`and ${errors.length - messages.length} more`);
